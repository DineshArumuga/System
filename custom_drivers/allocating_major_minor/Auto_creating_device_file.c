...
{
	pr_err("LDD session\n");
	if((alloc_chrdev_region(&dev,0,2,"Thunder"))<0)
	{
		pr_err("Cannot allocate major number for device\n");
		return -1;
	}
	pr_info("Major = %d Minor = %d\n",MAJOR(dev),MINOR(dev));
	dev_class = class_create(THIS_MODULE,"Soft");
	if(IS_ERR(dev_class))
	{
		pr_err("Cannot create the structure class for device\n");
		goto r_class;
	}
	if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"India")))
	{
...
MODULE_DESCRIPTION("SIMPLE DEVICE CREATE");
...
static int __init hello_world_init(void)
{
        ...
        //Creating device
        if(IS_ERR(device_create(dev_class,0,dev,0,"Dinesh"))){
            pr_err("Cannot create the Device\n");
            goto r_device;
        }
        pr_info("Kernel Module Inserted Successfully...\n");
